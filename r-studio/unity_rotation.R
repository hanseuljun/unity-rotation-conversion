...
rotate_vector_by_quaternion <- function(vx, vy, vz, qw, qx, qy, qz) {
  ...
  return(multiply_quaternions(qq[1], qq[2], qq[3], qq[4], qw, -qx, -qy, -qz))
}

convert_quaternion_to_look_at <- function(qw, qx, qy, qz) {
  qq = rotate_vector_by_quaternion(0, 0, 1, qw, qx, qy, qz)
  return(qq[2:4])
}

convert_quaternion_to_look_up <- function(qw, qx, qy, qz) {
  qq = rotate_vector_by_quaternion(0, 1, 0, qw, qx, qy, qz)
  return(qq[2:4])
}

qq <- convert_euler_to_quaternion(180.0, 0, 0)
convert_quaternion_to_look_at(qq[1], qq[2], qq[3], qq[4])